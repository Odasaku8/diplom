...
def home_view(request):
    ...
    return render(request, 'ecom/index.html', {'categories': categories, 'products': products, 'product_count_in_cart': product_count_in_cart})
...
def my_profile_view(request):
    ...
    return render(request, 'ecom/my_profile.html', {'customer': customer, 'profile_pic': profile_pic})
...
def get_product_recommendations(product, products, similarity_matrix, num_recommendations=5):
    ...
    # Get the top N most similar products (excluding the given product)
    top_indices = [index for index in sorted_indices if index != product_index][:num_recommendations]

    # Get the recommended products
    recommendations = [products[index] for index in top_indices]

    return recommendations


def product_recommendations_view(request, product_id):
    ...
    similarity_matrix = calculate_similarity(all_products)

    # Get product recommendations
    recommendations = get_product_recommendations(target_product, all_products, similarity_matrix)
    return  None
    #return render(request, 'ecom/product_recommendations.html', {'target_product': target_product, 'recommendations': recommendations})
